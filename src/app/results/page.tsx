    ...
    <div className="flex min-h-dvh flex-col bg-gradient-to-b from-gray-50 to-white text-gray-900">
        ...
        {loading ? (
          <div className="space-y-4">
            <ol className="space-y-1 text-sm">
              <li className={step >= 1 ? "text-gray-900" : "text-gray-400"}>1. Analyse de la photo</li>
              <li className={step >= 2 ? "text-gray-900" : "text-gray-400"}>
                2. Création des images ({imageStep}/{count})
              </li>
            </ol>
            <div className={`grid ${count === 2 ? "grid-cols-2" : "grid-cols-1"} gap-2`}>
              {Array.from({ length: count }).map((_, i) => (
                <div
                  key={i}
                  className="aspect-[4/5] w-full animate-pulse rounded-md bg-gray-200"
                />
              ))}
            </div>
          </div>
      ...
      <footer className="sticky bottom-0 border-t border-gray-200 bg-white/70 backdrop-blur">
        <div className="mx-auto max-w-screen-sm px-4 py-3 flex flex-col gap-2">
          ...
          <button className="w-full text-sm text-gray-600">Télécharger tout (ZIP)</button>
        </div>
      </footer>
    </div>
  );
